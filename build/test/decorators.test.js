...
describe('Request decorators', () => {
    test('logIn allows logging in an arbitrary user', async () => {
        ...
        server.get('/', { preValidation: fastifyPassport.authenticate('test', { authInfo: false }) }, async (request) => request.user.name);
    ...
    test('logIn allows logging in an arbitrary user for the duration of the request if session=false', async () => {
        const { server } = helpers_1.getConfiguredTestServer();
        server.post('/force-login', async (request, reply) => {
    ...
    test(`should logout`, async () => {
        const { server, fastifyPassport } = helpers_1.getConfiguredTestServer();
        server.get('/', { preValidation: fastifyPassport.authenticate('test', { authInfo: false }) }, async () => 'the root!');
        server.get('/logout', { preValidation: fastifyPassport.authenticate('test', { authInfo: false }) }, async (request, reply) => {
            void request.logout();
            void reply.send('logged out');
        });
        server.post('/login', { preValidation: fastifyPassport.authenticate('test', { successRedirect: '/', authInfo: false }) }, async () => '');
        const login = await server.inject({
            method: 'POST',
