...
test(`should return 200 if logged in against a running server`, async () => {
    ...
    const login = await got_1.default('http://localhost:' + port + '/login', {
        method: 'POST',
    ...
    const home = await got_1.default({
...
test(`should not log the user in when passed a callback`, async () => {
    const { server, fastifyPassport } = helpers_1.getConfiguredTestServer();
    server.get('/', { preValidation: fastifyPassport.authenticate('test', { authInfo: true }) }, async () => 'hello world!');
    server.post('/login', fastifyPassport.authenticate('test', async (request, reply, err, user) => {
        return user.name;
    }));
    const login = await server.inject({
        method: 'POST',
